...
{
    // The PE Header is not completely in the buffer, we need to extend it
    ...
    if needed_size > current_size {
        ...
        if needed_size > 650 * 1024 * 1024 { // Check if the needed size is more than 650MB
            log::warn!("Warning: Attempt to allocate overly large buffer of size {} at absolute offset 0x{:x}. Skipping...", needed_size, offset + nt_header_pos);
            continue;
        }

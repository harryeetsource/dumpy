    ...
    while (pos < size) {
      ...
      if (memcmp(dos_header, dos_magic, dos_magic_size) == 0) {
        ...
        if (pe_offset != 0 && pos + pe_offset + pe_magic_size <= size &&
          ...
          // Valid PE header not found, try to extract as raw data
          const char * file_data = & data[pos];
          std::string filename = output_path + std::to_string(count) + ".raw";
          std::ofstream output_file(filename, std::ios::binary);
          if (output_file) {
            uint32_t file_size = 0;
            if (pos + pe_offset + 0x50 <= size) {
              file_size = * reinterpret_cast <
                const uint32_t * > ( & data[pos + pe_offset + 0x50]);
            }
            if (pos + file_size <= size) {
              ...
              std::cerr << "Invalid size or size too large: " << file_size << std::endl;
              output_file.close();
              std::remove(filename.c_str());
            }
            count++;
            ...
            std::cerr << "Failed to open output file: " << filename << std::endl;
            pos += 1; // skip to next possible DOS header
          }
        }
      }
    }
